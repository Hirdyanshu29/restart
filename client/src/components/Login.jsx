...
function Login() {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        axios.post("http://localhost:8080/auth/login", document.querySelector("#login-form"), {
            headers: {
        ...
        <>
            ...
            <div className="flex bg-black/40 absolute flex-col items-center justify-center w-full h-screen z-50 overflow-hidden">
                <form id="login-form" className="bg-white flex flex-col items-center w-[90%] xs:w-[23rem] gap-4 p-8 rounded-2xl" onSubmit={handleSubmit} method="POST">
                    ...
                    <input type="text" onChange={(e) => setUsername(e.target.value)} value={username} name="username" placeholder="Enter Username user123" className="py-2 px-4 outline-2 outline-orange-400 shadow-[#0000004f] rounded-xl shadow-sm" />
                    <input type="password" onChange={(e) => setPassword(e.target.value)} value={password} name="password" placeholder="Enter Password 12345678" className="py-2 px-4 outline-2 outline-orange-400 shadow-[#0000004f] rounded-xl shadow-sm" />
